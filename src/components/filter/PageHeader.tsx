...
export default function PageHeader({
    ...
    return (
        <section className={cn('col-span-full flex items-center gap-2 max-w-full rounded-lg bg-white p-2 shadow', className)}>
            {back && (
                <BackButton back={back} className="bg-secondary-100 h-[60px] w-[60px] rounded" />
            )}

            <div className={cn('w-full grid grid-cols-[auto_1fr_auto_auto] h-[60px] rounded bg-secondary-100 p-3 gap-4', innerClassName)}>
                <button
                    type="button"
                    disabled={!search}
                    onClick={() => setSearchActive(true)}
                    className={cn('mr-auto flex items-center gap-4 transition-colors pr-6 pl-1 disabled:opacity-100 text-on-secondary-light', {
                    ...
                    {!isSearchActive && <h1 className="text-base text-on-secondary-light font-black">{title}</h1>}
                </button>
                ...
                {filteredActions.length > 0 && (
                    <div className="flex items-center gap-3 bg-secondary-100">
